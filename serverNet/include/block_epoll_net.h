...
public:
    ...
    {
        m_recv_callback = recv_callback;  //回调函数以函数指针的形式传入
        InitThreadPool();
        ...
        //set REUSERADDR   : O_REUSEADDR是让端口释放后立即就可以被再次使用。
        ret = setsockopt(m_listenfd, SOL_SOCKET, SO_REUSEADDR, (char *)&flags, sizeof(flags));  //SO_REUSEADDR：端口复用：允许一个进程中的多个socket绑定同一个端口（这个说法好像不太对）
        //相同协议的不同进程不能绑定同一个端口，但不同协议的进程：如一个tcp，一个udp可以绑定同一个端口
        if ( ret == -1 ){
