...
void Block_Epoll_Net::accept_event()
{
    ...
    int clientfd ;
    if ((clientfd = accept(m_listenfd, (struct sockaddr *)&caddr, &len)) == -1) { //清考虑如果为et类型的epoll，此处应该怎么编写比较合适
...
//    //设置非阻塞
//    setNonBlockFd( clientfd );   //int ret = fcntl(fd, F_SETFL, flags|O_NONBLOCK);  //对文件描述符进行操作，将其设置为非阻塞
...
void* Block_Epoll_Net::recv_task(void* arg)  //当epoll监听到某个socket可读后，调用此函数,参数为一个自定义结构体
{
    ...
    do
    {
        nRelReadNum = read(ev->fd,&nPackSize,sizeof(nPackSize) );  //先读包大小
        if(nRelReadNum <= 0)
            break;
        if(nPackSize == 1)
            continue;
        ...
        nRelReadNum = 0;
        //接收包的数据
        while(nPackSize)   //一直读直到读取数据的大小等于目标结构体的大小
        {
        ...
        pthis->m_threadpool->Producer_add(  Buffer_Deal , (void*) buffer );  //往线程池人物队列添加任务
...
void Block_Epoll_Net::setNonBlockFd(int fd)
{
    ...
    int ret = fcntl(fd, F_SETFL, flags|O_NONBLOCK);  //对文件描述符进行操作，将其设置为非阻塞
    if( ret == -1)
...
void Block_Epoll_Net::setNoDelay( int fd)
{
    //nodelay
    int value = 1;
    setsockopt(fd, IPPROTO_TCP, TCP_NODELAY ,(char *)&value, sizeof(int)); //TCP_NODELAY：禁用neggle算法
}
