...
{
    if((pthread_kill(tid,0)) == -1)   //在类unix系统中，线程本就是轻量级进程，所以可以通过此方法来判断县城是否存货
    {
        if(errno == ESRCH)
            return FALSE;
    }
...
void * thread_pool::Custom(void * arg)
{
    ...
    while(p->thread_shutdown)
    {
        ...
        {
            if(p->thread_wait > 0 && p->thread_alive > p->thread_min)   //在此处进行判断更加合理
            {
                --(p->thread_wait);
                --(p->thread_alive);
        ...
        ++(p->thread_busy);                //这里是否存在问题，只是指针指向，但是资源尚未读取过来，
                                                //是否可能导致被后面的数据覆写？
        pthread_mutex_unlock(&p->lock);
        //执行核心工作
        (*task.task)(task.arg);
...
void *thread_pool::Manager(void *arg)
{
    ...
    while(p->thread_shutdown )
    {
        ...
        if((cur > alive - busy || (float)busy / alive*100 >= (float)80 ) &&
                p->thread_max > alive)   //I/O密集型，可以考虑存活线程稍多一点
        {
