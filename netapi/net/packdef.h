...
struct STRU_ADD_FOLDER_RQ
{
    ...
    int timestamp;
    int userid;
    int flag;                        //用于表示是游戏的外层文件夹还是文件的内层文件夹,0表示为内层文件夹，1表示为外层文件夹
    char fileName[_MAX_PATH_SIZE];
    char dir[_MAX_PATH_SIZE];//路径
    char time[_MAX_SIZE]; //上传时间
};
...
struct STRU_ADD_FOLDER_RS
{
    STRU_ADD_FOLDER_RS(): type(_DEF_PACK_ADD_FOLDER_RS)
     ,timestamp(0) ,userid(0), result(1),flag(1){
...
#define _DEF_UPLOAD_GAME_LABEL_RQ (_DEF_PACK_BASE + 38)      //游戏标签
struct STRU_UPLOAD_GAME_LABEL_RQ{
    STRU_UPLOAD_GAME_LABEL_RQ():type(_DEF_UPLOAD_GAME_LABEL_RQ){
...
struct STRU_GET_GAME_DREAM_RQ{
    ...
    PackType type;
    int userid;
    int dreamNum;            //dreamNum表示当前客户端已经有多少条愿望了
};
